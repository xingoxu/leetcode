...
public:
  ...
  {
    if (k >= prices.size() / 2)
      return maxProfit_everyStep(prices);

    int pricesSize = prices.size();

    vector<vector<int>> dp(k + 1);

    dp[0].resize(pricesSize + 1);

    for (int i = 1; i <= k; i++)
    {
      dp[i].resize(pricesSize + 1);
      int soldMaximum = 0;
      for (int j = 1; j <= pricesSize; j++)
      {
        int no_sold = dp[i][j - 1];
        if (j >= 2)
        {
          int onePriceDifferenceBefore = prices[j - 1] - prices[j - 2];
          soldMaximum = max(soldMaximum + onePriceDifferenceBefore, dp[i - 1][j - 2] + onePriceDifferenceBefore);
        }

        dp[i][j] = max(no_sold, soldMaximum);
      }
    }

    return dp[k][pricesSize];
  }
  int stillCanbeOptimized_maxProfit(int k, vector<int> &prices)
  {
    if (k >= prices.size() / 2)
      return maxProfit_everyStep(prices);
    ...
    for (int i = 1; i <= k; i++)
    {
      ...
      for (int j = 1; j <= pricesSize; j++)
      {
        ...
        for (int a = 1; a < j; a++)
        {
          childMaxium = max(childMaxium, dp[i - 1][a - 1] + prices[j - 1] - prices[a - 1]);
        }
        dp[i][j] = max(left, childMaxium);
      }
    }
  ...
  int maxProfit_everyStep(vector<int> &prices)
  {
    int pricesSize = prices.size();
    int sum = 0;
...
int main()
{
  ...
  // vector<int> prices = {2, 4, 1, 6, 7, 8, 9};
  cout << Solution().maxProfit(29, prices)
       << endl;
