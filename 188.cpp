...
public:
  int maxProfit(int k, vector<int> &prices)
  {
    ...
    // prices.resize(pricesSize - 1);

    return r_maxProfit(k, prices, 0, pricesSize - 2);
  }
  unordered_map<string, int> cache;
  int r_maxProfit(int k, vector<int> &difference, int left, int right)
  {
    // cout << k << ' ' << left << ' ' << right << endl;
    string key = to_string(k) + "#" + to_string(left) + "#" + to_string(right);
    ...
    if (k == 1)
    {
      const int &result = maxProfit(difference, left, right);
      cache[key] = result;
      return result;
    }

    int result = 0, leftBiggestResult = 0, leftSum = 0;
    for (int i = left; i <= right - k + 1; i++)
    {

      int &now = difference[i];
      leftSum += now;
      if (leftSum > leftBiggestResult)
      {
        leftBiggestResult = leftSum;
      }
      if (leftSum < 0)
      {
        leftSum = 0;
      ...
      int childResult = leftBiggestResult + r_maxProfit(k - 1, difference, i + 1, right);
